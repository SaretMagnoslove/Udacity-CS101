...
def recip(graph, source, destination, k):
    if k==0: return True if source==destination else False
    if source in graph[destination]: return True
    for node in graph[destination]:
        if recip(graph, source, node, k-1): return True
    return False
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    if not recip(graph, node, page, k):
